...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'items' AND column_name = 'added_date') THEN
    ALTER TABLE items ADD COLUMN added_date TIMESTAMPTZ DEFAULT NOW();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'items' AND column_name = 'last_used_date') THEN
    ALTER TABLE items ADD COLUMN last_used_date TIMESTAMPTZ;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'items' AND column_name = 'vehicle_model') THEN
    ALTER TABLE items ADD COLUMN vehicle_model TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'items' AND column_name = 'store_location') THEN
    ALTER TABLE items ADD COLUMN store_location TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'items' AND column_name = 'added_by') THEN
    ALTER TABLE items ADD COLUMN added_by UUID REFERENCES auth.users(id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'items' AND column_name = 'last_used_by') THEN
    ALTER TABLE items ADD COLUMN last_used_by UUID REFERENCES auth.users(id);
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'item_transactions' AND column_name = 'performed_by') THEN
    ALTER TABLE item_transactions ADD COLUMN performed_by UUID REFERENCES auth.users(id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'item_transactions' AND column_name = 'vehicle_model') THEN
    ALTER TABLE item_transactions ADD COLUMN vehicle_model TEXT;
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  email TEXT NOT NULL,
  full_name TEXT,
  role TEXT NOT NULL DEFAULT 'worker' CHECK (role IN ('super_admin', 'admin', 'inventory_manager', 'worker')),
  address TEXT,
  phone_number TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE POLICY "Admins can read all profiles" ON profiles FOR SELECT USING (public.get_my_role() IN ('super_admin', 'admin'));
CREATE POLICY "Admins can insert profiles" ON profiles FOR INSERT WITH CHECK (public.get_my_role() IN ('super_admin', 'admin'));
CREATE POLICY "Admins can update profiles" ON profiles FOR UPDATE USING (public.get_my_role() IN ('super_admin', 'admin'));
...
DECLARE user_role TEXT; invite_token TEXT;
BEGIN
  invite_token := NEW.raw_user_meta_data->>'invite_token';
  IF invite_token IS NOT NULL THEN
    SELECT role INTO user_role FROM public.user_invites WHERE token = invite_token AND email = NEW.email;
    IF user_role IS NOT NULL THEN
      DELETE FROM public.user_invites WHERE token = invite_token;
    ELSE user_role := 'worker';
    END IF;
  ELSE
    SELECT CASE WHEN (SELECT COUNT(*) FROM public.profiles) = 0 THEN 'super_admin' ELSE 'worker' END INTO user_role;
  END IF;
  INSERT INTO public.profiles (id, email, full_name, role)
  VALUES (NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email), user_role)
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.setup_complete() RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
...
CREATE POLICY "Authenticated upload item-photos" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'item-photos' AND auth.role() = 'authenticated');
