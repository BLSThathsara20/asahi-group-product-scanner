...
export function AddItem() {
  ...
  useEffect(() => {
    ...
    if (barcode) {
      try {
        setForm((prev) => ({ ...prev, barcodes: [decodeURIComponent(barcode)] }));
      } catch {
        setForm((prev) => ({ ...prev, barcodes: [barcode] }));
      }
    }
  }, [searchParams]);
  ...
  /** Remove repeated scans (e.g. 89411540314778941154031477 → 8941154031477) */
  const normalizeRepeatedBarcode = (value) => {
    const trimmed = String(value || '').trim();
  ...
  const allBarcodes = form.barcodes.map((b) => extractBarcode(normalizeRepeatedBarcode(b))).filter(Boolean);
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const bases = allBarcodesTrimmed.map((b) => extractBarcode(normalizeRepeatedBarcode(b)));
                ...
                <div key={index} className="flex gap-2 items-center">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        const currentValue = e.target.value ?? barcode;
                        const normalized = normalizeRepeatedBarcode(currentValue);
                        if (normalized !== currentValue) {
                          handleBarcodeChange(index, normalized);
                          setBarcodeError('');
                        }
                      }
                    }}
